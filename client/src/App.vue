    ...
    <section class="grid grid-cols-2 gap-6 bg-gray-50 p-6 rounded-xl shadow">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          ...
          <button class="text-sm px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600" @click="uploadSlide">
            Upload Slide
          </button>
        </div>
        <div class="h-[400px] overflow-y-auto border-gray-200  rounded p-2 flex flex-wrap gap-6 justify-start
                    scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
          <SlideCard v-for="item in contentList" :key="item.id" :item="item" :selected="selectedContent?.id === item.id"
      ...
      <div class="flex flex-col gap-4">
        <div>
          ...
          <button class="px-6 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 mb-2" @click="addSlidesGroup">
            Add Slides Group
          </button>
