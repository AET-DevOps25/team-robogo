  ...
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
    ...
    <section>
      ...
      <div
        ...
        <div class="flex flex-wrap gap-6 justify-start">
          <ScreenCard v-for="screen in screens" :key="screen.id" :screen="screen" :slide-groups="slideGroups"
            :all-slides="slides" @update-group="onUpdateScreenGroup" @request-delete="
              (screen: Screen) => {
    ...
    <section class="grid grid-cols-[2fr_1fr] gap-6 bg-gray-50 dark:bg-gray-800 p-6 rounded-xl shadow">
      ...
      <div>
        ...
        <div>
          <USelectMenu v-model="selectedGroupId" :items="slideGroups.map((g: SlideGroup) => g.id)" class="w-48" />
        ...
        <div
          ...
          <SlideGroupCard v-if="currentGroup && currentGroup.id !== 'None'" :key="currentGroup.id"
            v-model:slide-ids="currentGroup.slideIds" v-model:speed="currentGroup.speed" :title="currentGroup.id"
            :selected-content="selectedContent" @select="selectContent" />
        </div>
      </div>
    ...
    <section class="bg-white dark:bg-gray-700 p-4 rounded-xl shadow-md flex flex-col sm:flex-row items-center gap-4">
      <button
        class="px-4 py-2 bg-green-600 dark:bg-green-500 text-white rounded hover:bg-green-700 dark:hover:bg-green-600"
        @click="callTestApi">
        {{ t('test') }}
...
watchOnce(
  ...
  () => {
    if (!store._timer) {           //  _timer 为 null 才启动
      console.log('🔄 Timer start triggered')
